...
static volatile uint8_t s_usbVideoTxBusy = 0U;
...
void USB_VIDEO_CDC_Init(void)
{
	APPInit();
}

void USB_VIDEO_CDC_Task(void)
{
    APPTask();
}

bool USB_VIDEO_CDC_IsReady(void)
{
    return ((1U == s_cdcVcom.attach) && (1U == s_cdcVcom.startTransactions));
}

bool USB_VIDEO_CDC_CanSend(void)
{
    return (USB_VIDEO_CDC_IsReady() && (0U == s_usbVideoTxBusy));
}

bool USB_VIDEO_CDC_Send(uint8_t *buffer, uint32_t length)
{
    usb_status_t error;
